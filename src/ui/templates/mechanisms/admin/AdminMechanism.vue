<template>
  <Pane @close-pane="emit('close-pane', $event)">
    ...
    <template #tool>
      <template v-if="actual && actual.spaceId">
        <el-tooltip placement="top" content="Go to space">
          <router-link :to="`/app/manage/${actual.spaceId}`" class="el-button -square -common-height -borderless">
            <Icon type="map" />
          </router-link>
        </el-tooltip>
      </template>
      ...
      <div class="admin-single-grid grid">
        <div>
          ...
          <div v-if="creating" class="m-up text-center">
            <el-button :disabled="isSubmitting" @click="create">Create mechanism</el-button>
          </div>
        </div>
